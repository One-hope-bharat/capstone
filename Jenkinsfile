	...
	stage('deploy to EC2 '){
		steps{
			sshagent(['54.185.10.226']) {
				...
				sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 54.185.10.226 "bash -l -c \'echo "$DOCKERHUB_CREDENTIALS_PSW" | docker login -u "$DOCKERHUB_CREDENTIALS_USR" --password-stdin\'"'
